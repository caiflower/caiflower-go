...
//
//
// Related Topics 树 深度优先搜索 二叉搜索树 二叉树 👍 1079 👎 0
...
type convertBSTStack []*TreeNode

func (s *convertBSTStack) Push(x *TreeNode) {
	*s = append(*s, x)
}

func (s *convertBSTStack) Pop() *TreeNode {
	v := (*s)[len(*s)-1]
	*s = (*s)[:len(*s)-1]
	return v
}

func (s *convertBSTStack) Size() int {
	return len(*s)
}
